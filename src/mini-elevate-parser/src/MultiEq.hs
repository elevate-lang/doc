...
data KindRep = Type | RowLack [Label] | RowPres [Label] deriving (Show, Eq, Ord)

data TIdRep = TIdRep Id KindRep deriving (Show, Eq, Ord)

data TypeRepF t = 
  IdTypeRep TIdRep |
  FunTypeRep t t |
  RowRep (Map.Map Label t) TIdRep |
  VariantRep t |
  RecordRep t
...
data Descriptor = Desc {
  structure :: IORef (Maybe Structure),
  rank :: IORef Int,
  mark :: IORef (),
  kind :: IORef VariableKind,
  name :: IORef (Maybe Id),
  var :: IORef (Maybe Variable)
  }

data VariableKind = Rigid | Flexible

type Structure = TypeRepF Variable
